...
export default function Card({seccion, index, activeSlide, slideStyles, next, prev}: Card) {
  return (
    <div className={`text-white relative ${activeSlide === index ? 'z-20' : 'z-10'} ${slideStyles(index)} 
    ${(index === 0 && activeSlide === 0) ? 'col-start-2' : 'col-start-auto' }`}>
        <figure className='aspect-[4/5] h-[460px] mask-b-from-50% mask-b-to-90%'>
           <img src={seccion.img} alt={`Imagen de la sección ${seccion.title}`} className='size-full object-cover' />
        </figure>
        <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 sm:translate-y-1/3 translate-y-20
        md:w-3xl px-5 text-center flex flex-col justify-center items-center gap-2
        ${index === activeSlide ? '' : 'hidden'}`}>
            <h2 className='md:text-5xl text-4xl font-black tracking-widest uppercase'>
                {seccion.title}
            </h2>
            <p className='md:text-lg text-wrap'>
                {seccion.content}
            </p>
